#include <task/template.hpp>

namespace hbt::mods::task {
[[nodiscard]] auto Template::fromValidated(uuid_t uuid, std::string title,
...
Template::Template(Validator::Validated, uuid_t uuid, std::string title,
...
Template::Template(std::string title, util::Recurrence recurrence,
                   Deadline deadline)
...
[[nodiscard]] auto Template::getUUID() const -> uuid_t { return uuid_; }

[[nodiscard]] auto Template::getStartDateTime() const -> DateTime {
    return recurrence_.getStartDateTime();
}

[[nodiscard]] auto Template::getEndDateTime() const -> OptDateTime {
    return recurrence_.getEndDateTime();
}

[[nodiscard]] auto Template::getRecurrence() const -> util::Recurrence {
    return recurrence_;
}
...
auto Template::setStartDateTime(const DateTime &startDateTime) -> void {
    try {
...
auto Template::setRecurrence(const util::Recurrence &recurrence) -> void {
    recurrence_ =
...
auto Template::setEndDateTime(const OptDateTime &endDateTime) -> void {
    try {
...
[[nodiscard]] auto Template::generateInstancesForDate(Date date) const
...
[[nodiscard]] auto Template::happensOnDate(Date date) const -> bool {
    return recurrence_.happensOnDate(date);
}

/* ------- JSON ------- */
[[nodiscard]] auto Template::JSON::containsAllFields(const nlohmann::json &json)
    -> bool {
...
[[nodiscard]] auto Template::JSON::encode(const Template &taskSeries)
    -> nlohmann::json {
    return {
...
[[nodiscard]] auto Template::JSON::decode(const nlohmann::json &json)
    -> std::expected<Template, JSON::Error> {
...
[[nodiscard]] auto Template::Validator::Return::recurrence(
